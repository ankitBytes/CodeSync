      ...
      reminders: [
        {
          type: {
            type: String,
            enum: ["session_start", "break_time", "session_end"],
          },
...
sessionSchema.virtual("activeParticipantsCount").get(function () {
  return this.participants.filter(
    (p) => new Date() - new Date(p.lastActive) < 5 * 60 * 1000 // 5 minutes
  ).length;
});
...
sessionSchema.virtual("isCompleted").get(function () {
  return (
    ...
    this.execution.passedTests === this.execution.totalTests
  );
});
...
sessionSchema.virtual("progress").get(function () {
  ...
  return Math.round(
    (this.execution.passedTests / this.execution.totalTests) * 100
  );
});
...
sessionSchema.pre("save", function (next) {
  if (this.isModified("execution.testResults")) {
    ...
    this.execution.passedTests = this.execution.testResults.filter(
      (t) => t.passed
    ).length;
    this.execution.failedTests =
      this.execution.totalTests - this.execution.passedTests;
  }
...
sessionSchema.methods.addParticipant = function (
  ...
  if (
    this.participants?.find((p) => p.userId?.toString() === userId.toString())
  ) {
...
sessionSchema.methods.endSessionByCreator = function (userId) {
...
sessionSchema.methods.updateCode = function (
  newCode,
  userId,
  changeDescription = ""
) {
